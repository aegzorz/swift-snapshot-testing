...
extension Snapshotting where Value == CALayer, Format == UIImage {
  ...
  ///
  /// - Parameter precision: The percentage of pixels that must match.
  public static func image(precision: Float = 1, strategy: ImageDiffingStrategy = .perImage, traits: UITraitCollection = .init())
    -> Snapshotting {
      return SimplySnapshotting.image(precision: precision, strategy: strategy).pullback { layer in
