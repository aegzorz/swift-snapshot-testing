...
extension Snapshotting where Value == CGPath, Format == UIImage {
  ...
  ///
  /// - Parameter precision: The percentage of pixels that must match.
  public static func image(precision: Float = 1, strategy: ImageDiffingStrategy = .perImage, scale: CGFloat = 1, drawingMode: CGPathDrawingMode = .eoFill) -> Snapshotting {
    return SimplySnapshotting.image(precision: precision, strategy: strategy).pullback { path in
      let bounds = path.boundingBoxOfPath
